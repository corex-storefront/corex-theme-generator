...
const {
  ColorsCx: { td, bg, color, colorTile }
} = require("../style");
...
const ColorsTable = config =>
  e(
    ...
    e(
      "tbody",
      { className: "f6", cellspacing: 0, cellpadding: 0 },
      Colors(config)
    )
  );
...
const Colorable = colors =>
  e(
    "div",
    { className: "cf w-100 flex flex-wrap" },
    colorable(colors, { threshold: 3 }).map(combo =>
      e(
        "div",
        { key: combo.name, className: `${colorTile} bg-${combo.name}` },
        combo.combinations.map(color =>
          e(
            "div",
            { key: `${color.name}${combo.name}`, className: color.name },
            e(
              "h1",
              null,
              "Aa",
            ...
            e(
              "p",
              { className: "f6 lh-copy code" },
              `.bg-${combo.name} + .${color.name}`
            ),
...
module.exports = (config, typeScale) =>
  e(
    ...
    Section({
      title: "Colors",
      children: [HtmlSection(ColorsTable(config)), CodeSection(typeScale)]
    }),
    Section({
