...
module.exports = colors => {
  ...
  const colorsWithTransparency = Object.assign({}, colors, {
  ...
  colorGenerator.skinsPseudo = () =>
    skinsPseudo(colors, colorsWithTransparency);
...
const hoverBg = colorToCss(key => [
  `.hover-bg-${key}:focus, .hover-bg-${key}:hover { background-color: ${asVar(
    key
  )} }`
]);
...
const listToMap = list =>
  list.reduce((acc, obj) => Object.assign(acc, obj), {});
