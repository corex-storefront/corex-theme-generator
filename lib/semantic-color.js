module.exports = colors => {
  colorGenerator.backgroundColors = () =>
    appendTransparentBackground(bg("bg", colors.background));
  colorGenerator.skinsPseudo = () => skinsPseudo(colors);
  colorGenerator.borderColors = () =>
    appendTransparentBorder(border("b", colors.border));
...
const colorToCss = fn => (prefix, obj) =>
  Object.keys(obj)
    .map(key => fn(prefix, key))
    .join("\n");
...
const text = colorToCss(
  (prefix, key) => `.${prefix}-${key} { color: ${asVar(prefix, key)} }`
);

const bg = colorToCss(
  (prefix, key) =>
    `.${prefix}-${key} { background-color: ${asVar(prefix, key)} }`
);

const border = colorToCss(
  (prefix, key) => `.${prefix}--${key} { border-color: ${asVar(prefix, key)} }`
);

const hoverText = colorToCss((prefix, key) => [
  `.${prefix}-${key}:focus, .${prefix}-${key}:hover { color: ${asVar(
    prefix,
    key
  )} }`
]);
...
const hoverBg = colorToCss((prefix, key) => [
  `.${prefix}-${key}:focus, .${prefix}-${key}:hover { background-color: ${asVar(
    prefix,
    key
  )} }`
]);
...
const hoverB = colorToCss((prefix, key) => [
  `.${prefix}--${key}:focus, .${prefix}--${key}:hover { border-color: ${asVar(
    prefix,
    key
  )} }`
]);
...
const addVariables = (prefix, colors) =>
  Object.keys(colors).reduce(
    (acc, key) => acc.concat(`  --${prefix}-${key}: ${colors[key]};`),
    []
  );
